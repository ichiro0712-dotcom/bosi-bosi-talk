    ...
    <>
      <div className="chat-area glass-panel">
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {messages.map((msg) => (
            <div key={msg.id} style={{ alignSelf: msg.isMine ? 'flex-end' : 'flex-start', display: 'flex', flexDirection: 'column', alignItems: msg.isMine ? 'flex-end' : 'flex-start' }}>
              <div style={{
                background: msg.isMine ? 'var(--primary)' : 'rgba(255, 255, 255, 0.95)', padding: '12px 16px', borderRadius: '18px',
                borderBottomRightRadius: msg.isMine ? '4px' : '18px', borderBottomLeftRadius: msg.isMine ? '18px' : '4px', maxWidth: '75%',
                color: 'var(--text-main)', boxShadow: '0 4px 12px rgba(100, 116, 166, 0.08)'
              }}>
                {msg.text && <div style={{ wordBreak: 'break-word', lineHeight: '1.4', fontWeight: 500 }}>{msg.text}</div>}
