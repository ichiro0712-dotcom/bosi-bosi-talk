...
const CAPABILITIES: CapabilityCategory[] = [
  {
    ...
    items: [
      ...
      {
        title: 'リマインダーのセット',
        desc: '時間や曜日を指定すると、その時刻にプッシュ通知を送ってくれる。1回・毎日・毎週に加え、「それ以上」ボタンから 毎月○日 / 毎月第○△曜日 / Nヶ月に1回 (2〜120ヶ月の任意の数字、 24・36 ヶ月なども OK、 開始年月も指定可) を設定できる。',
        examples: [
          '「毎朝 7 時に水を飲むようリマインドして」',
          '「毎週月曜の朝にゴミ出し」',
          'TODO 画面で「それ以上」→ サブモーダルで「それ以上」 → 「2026 年 7 月から 3 ヶ月に 1 回」+ 日にち or 曜日を指定',
        ],
      },
      {
        title: '週初め・月初めの予定まとめ',
        desc: '毎週月曜 10 時には今週のタスクと、 今週発火する毎週/毎月リマインダーを教えてくれる。 毎月 1 日 10 時には来月の予定と、 月次以上のリマインダー (毎月・N ヶ月に 1 回) を教えてくれる。',
        examples: [
          '月曜の朝にもちから「今週はこれだね！」とメッセージが来る',
          '月初の朝にもちから「来月はこれが予定だよ」とメッセージが来る',
        ],
      },
      {
        title: '今日のご飯を提案',
  ...
  },
  {
    ...
    items: [
      ...
      {
        title: '食事の記録 (カロリー・栄養素)',
        desc: 'チャット下の「ツール」から「食事記録」を選んで食べたものを送ると、Agent Hub がカロリーと栄養素を計算してスプレッドシートに記録。今日の合計・ここ 1 週間の傾向・次の食事のアドバイスが少ししてから届く。記録はミルクとメリーで別々のシートに分かれる。',
        examples: [
          '「唐揚げとサラダ食べた」',
          '「お昼にラーメン 1 杯」',
          '「20 時にカレー食べた」',
  ...
  },
  {
    ...
    items: [
      {
        title: '「ツール」ボタンで用途を指定',
        desc: 'チャット入力欄の下にある「ツール」ボタンから、もち / 食事記録 / リマインダー / スケジュール を選べる。選ぶとその用途に決まった動きをするので、普通に話しかけるより確実。もう一度押すと解除。',
        examples: [
          'ツール →「リマインダー」→「明日 8 時にゴミ出し」',
          'ツール →「スケジュール」→「8月3日 19時に歯医者」',
          'ツール →「食事記録」→「唐揚げ食べた」',
        ],
      },
    ],
